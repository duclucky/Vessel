...
/**
 * Real StorageProvider backed by Shelby **testnet** via **Solana Derived Account Abstraction**.
 * A Solana keypair controls the Aptos storage account that owns every blob — the DAA
 * differentiator, fully realized (verified end-to-end: NOTES.md 5g). This is the only
 * published combo where DAA byte-upload works: solana-kit@0.2.8 + sdk@0.3.1 + Network.TESTNET
 * (no `location` arg — testnet's contract predates it). Reads/challenge are anonymous on
 * testnet, but we send the API key when present to lift rate limits.
 * @implements {import('./types.js').StorageProvider}
 */
export class ShelbyProvider {
  constructor({ apiKey, solanaSecretKey, domain = 'vessel.demo', publicBase }) {
  ...
  rawReadUrl(key) { return `https://api.testnet.shelby.xyz/shelby/v1/blobs/${this.address.toString()}/${key}`; }

  async health() {
    try {
      const apt = await this.client.aptos.getAccountAPTAmount({ accountAddress: this.address });
      return { ok: apt >= 10_000_000, network: 'testnet', chain: 'solana-DAA', account: this.address.toString(), solana: this.keypair.publicKey.toBase58(), aptOctas: apt };
    } catch (e) {
      return { ok: false, network: 'testnet', account: this.address.toString(), error: String(e?.message || e).slice(0, 120) };
    }
  }
